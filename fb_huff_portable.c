 ...
 * Subject to the GPL.
 *
 * NOTE: Data is supposed to arrive in Big Endian Order into the module!
 */
					 ...
					 struct code_book *code_en)
{
	...
	if (freebits < 8) {
		...
		bitstream = htonl(bitstream);
		memcpy(tempout, &bitstream, 3 * sizeof(char));
		counter += 3;	
	}
	else if (freebits >= 16 && freebits < 24) {
		bitstream = htonl(bitstream);
		memcpy(tempout, &bitstream, 2 * sizeof(char));
		counter += 2;
	}
	else if (freebits >= 24 && freebits < 32) {
		bitstream = htonl(bitstream);
		memcpy(tempout, &bitstream, 1 * sizeof(char));
		counter += 1;
	}
	/*if (freebits != 32) {
		memcpy(tempout, &bitstream, 4 * sizeof(char));
